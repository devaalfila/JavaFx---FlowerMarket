...
public class TransactionFormAdmin {
	private Stage stage;
	private Scene scene;
	...
	@FXML
	private TableColumn<Transaction, String> buyer;
	@FXML
	...
	private void loadData() {
		...
		buyer.setCellValueFactory(f -> new SimpleStringProperty(f.getValue().getUserID()));
	...
	private void refreshTableTransaction() {
		...
		String query = "SELECT *, item.quantity AS itemQuantity, transactiondetail.quantity AS transactionQuantity FROM `transaction` LEFT JOIN transactiondetail USING(transactionID) LEFT JOIN item USING(itemID) LEFT JOIN user USING(userID)";
...
//				LocalDate ld = LocalDate.parse(rs.getString("transactionDate"), dt_format);
	...
	public String getBuyerName(String transaction_id) {
		Database dc = new Database();
		Connection conn = dc.getConnection();
		String query = "SELECT * FROM `transaction` LEFT JOIN user USING(userID) WHERE transactionID = ? LIMIT 1";
		PreparedStatement stmt;
		try {
			stmt = conn.prepareStatement(query);
